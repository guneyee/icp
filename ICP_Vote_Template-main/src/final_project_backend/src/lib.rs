...
impl Storable for Proposal {
    ...
    fn from_bytes(bytes: Cow<[u8]>) -> Self {
        candid::decode(bytes.as_ref()).unwrap()
    }
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        let mut old_proposal_opt = p.borrow_mut().get_mut(&key);
...
fn end_proposal(key: u64) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        if let Some(proposal) = p.borrow_mut().get_mut(&key) {
...
fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        let mut proposal_opt = p.borrow_mut().get_mut(&key);
